...
func TestRunSingleEnv(t *testing.T) {
	...
	aggs, err := runSingleEnv(ctx, probes, "before", 1, 4, fakeRun, nil)
	if err != nil {
...
func TestRunSingleEnv_DeterministicAcrossConcurrency(t *testing.T) {
	...
	a1, err := runSingleEnv(ctx, probes, "before", 3, 1, fakeRun, nil)
	if err != nil {
		t.Fatalf("concurrency 1: %v", err)
	}
	a8, err := runSingleEnv(ctx, probes, "before", 3, 8, fakeRun, nil)
	if err != nil {
