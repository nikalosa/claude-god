...
- *Implementation note (#9):* the median-then-threshold is realized as a **per-run** threshold (`score ≥ pass_score`) collapsed by the aggregator's existing majority vote — which equals median-of-scores thresholding for **odd N** (N=3, adaptive N=5), and only for odd N. `--samples` is therefore required to be odd, the judge is called exactly once per sampled answer, and the deterministic aggregator needs no judge-specific path.
