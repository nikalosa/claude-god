...
- **Deliberately untouched:** the generic `validateSamples` / `validateConcurrency` helpers (input validation, not the tool name); the recorded `internal/parser/testdata` fixtures (opaque captured payload). The bare-command file `internal/cli/evaluate.go` was renamed to `bare.go` (its symbol was already `defaultRunE`, so no code changed); only ADR-0008's "Evaluation" title/framing still carries the third root, and reconciling that is deferred, not blocked by this rename.
