...
- **Grading runs in a second bounded pool.** The Judge also shells `claude -p`, so after the run pool drains the probes are graded in their own `errgroup` capped by the same `--concurrency` — only the judge calls (`Prefer`, `Score`) overlap, while the pure-CPU aggregation rides along inside each per-probe task. The verdict is preserved the same way the run pool preserves it: results are written by probe index and preferences compacted in probe order after the pool drains, so completion order never changes the report (unit-tested identical at `--concurrency 1` vs 8 with a stub judge). First grade error cancels in-flight judge calls; a still-failing preference is report-only and dropped per-probe, never aborting.
